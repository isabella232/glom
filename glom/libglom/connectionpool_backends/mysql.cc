...
bool MySQL::create_text_file(const std::string& file_uri, const std::string& contents, bool current_user_only)
{
  ...
  try
  {
    if(file->query_exists())
    {
      ...
      {
        stream = file->replace(std::string() /* etag */, false /* make_backup */, Gio::File::CreateFlags::PRIVATE); //Instead of append_to().
      }
      else
      {
    ...
    {
      //By default files created are generally readable by everyone, but if we pass File::CreateFlags::PRIVATE in flags the file will be made readable only to the current user, to the level that is supported on the target filesystem.
      if(current_user_only)
      {
